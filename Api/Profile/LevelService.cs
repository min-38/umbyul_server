...
/// XP 집계는 ReviewerLevel과 동일(리뷰·받은 좋아요·픽 당일 리뷰). 새 테이블/캐시 컬럼 없이 배치 쿼리 1회.
        ...
        NpgsqlConnection conn, IReadOnlyCollection<Guid> ids, CancellationToken ct)
    {
        ...
        foreach (var sql in new[] { withPicks, noPicks })
        {
            try
            {
                ...
                {
                    var id = rd.GetGuid(0);
                    var reviews = (int)rd.GetInt64(1);
                    var likes = (int)rd.GetInt64(2);
                    var picks = (int)rd.GetInt64(3);
                    result[id] = ReviewerLevel.LevelFor(ReviewerLevel.Xp(reviews, likes, picks));
                }
                return result;
            }
            catch (NpgsqlException) { /* 픽 없이 재시도, 그래도 실패면 빈 맵 → 호출부 기본 Lv 1 */ }
        }
        result.Clear();
        return result;
    }
